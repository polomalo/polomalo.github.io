    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            fetch(sapi)
                ...
                .then(newData => {
                    console.log(newData);
                    const { name, region } = newData.location;
                    const { temp_c } = newData.current;
                    const { icon } = newData.current.condition;
                    cityName.innerHTML = `<h1>${name}</h1>`;
                    locationTimezone.innerHTML = `<h2>( ${region} )</h2>`;
                    temperatureDegree.innerHTML = temp_c;
                    Icon.innerHTML = `<img src="https:${icon}">`;
